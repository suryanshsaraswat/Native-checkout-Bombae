...
(function( $ ) {
   "use strict";
    ...
    if ($('.template-search .filter_area_js').length > 0) {
      type_shop_filters = false;
    }
...
//         previousScroll = currentScroll;
    ...
    geckoShopify.touchMegaMenu = function() {
      ...
      $(document).on('click', function (e) {
        ...
        if ($('.' + openedClass).length > 0 && !$(target).parents().is('.' + openedClass + '')) {
          mainMenu.find('.' + openedClass + '').removeClass(openedClass);
          h_cat_nav.find('.' + openedClass + '').removeClass(openedClass);
          return false;
        }
      });
      ...
      h_cat_nav.on('click', '.has-children>a', function (e) {
        //console.log('click',$(this),$(this).siblings('.sub-menu'))
        if ($(this).siblings('.sub-menu').length == 0) return;
          e.preventDefault();
          //e.stopPropagation();
  ...
  function getParameterByName(name, url) {
   ...
   geckoShopify.replaceStateURL = function(url) {
    ...
    geckoShopify.flickityt4sResposition = function (bl,el) {

      if (  $('.nt_products_holder.nt_slider').length == 0 || typeof ($.fn.flickityt4s) == 'undefined' ) return;
      
        var $grid = el || $('.nt_products_holder.flickityt4s-enabled');
         
         if (bl) {
           $grid.find('.swatch__list_js.lazyloadt4sed').each(function() {
               $grid.flickityt4s('resize')
           });
         }
   ...
   geckoShopify.productSticky = function () {
      ...
      if ( !body.hasClass('template-product') || $('.sticky_des_true').length == 0 || small767 ) return;
   ...
   geckoShopify.inc_lz = function () {
      var $inc_lz = $('.inc_lz');
      //Disable on designMode to not error HTML error found: admin Broken HTML has been detected in your theme...
      if ( $inc_lz.length == 0 && $('.inl_cnt_js').length > 0 || designMode ) return;
   ...
   geckoShopify.VariantFilterRefresh = function () {
    if (type_shop_filters) return;
     ...
     var Getsortby = getParameterByName('sort_by'),
        vl = 'sort_by='+Getsortby,
        url = window.location.href;
    
    if (Getsortby) {
       
      $('.nt_ajaxsortby a').each(function() { 
        $(this).attr( 'href',url.replace(vl,'sort_by='+$(this).attr("data-vl") ) );
      }); 

    } else {
       
        $('.nt_ajaxsortby a').each(function() { 
        $(this).attr( 'href',UpdateUrlFilter(url,'sort_by='+$(this).attr("data-vl"),'?') );
      });
    ...
    //    body: JSON.stringify({
    //     attributes: {
    //      cat_mb_items_per_row:'',
    //      cat_tb_items_per_row:'',
    ...
    // .then(data => {
    //   console.log('Success remove attributes:', data);
    // });
    ...
    function UpdateUrlFilter(href,prefix,strcheck) {
        href = href.split('#')[0]
       if (href.indexOf(strcheck) !== -1) {
           return href+'&'+prefix;
       } else {
           return href+'/?'+prefix;
       }
   ...
   geckoShopify.FilterUpdateJS = function () {
      ...
      var l_href = location.href,
          url1 = UpdateUrlFilter(l_href,'section_id=nt_filter2','?'),
          url2 = UpdateUrlFilter(l_href,'section_id=sidebar_shop2','?');

      if (type_shop_filters) {
        if (type_filters == '2') {
           
           $('.filter_area_js').attr('data-rendert4s',url1);
           $('.js_sidebar').attr('data-rendert4s',url2);
       ...
       $('.filter_area_js').attr('data-rendert4s',url1);
       $('.js_sidebar').attr('data-rendert4s',url2);
       $('.filter_area_js:not(.lazyloadt4sed),.js_sidebar:not(.lazyloadt4sed)').addClass('lazyloadt4s');
  ...
  geckoShopify.ClearFilterStockt4 = function () {
    ...
    body.on('click', '.price_slider_btn', function (e) {
          ...
          var _this = $(this),
              ...
              minstr0 = _amount.data('minstr'),
              maxstr0 = _amount.data('maxstr'),
              minstr = minstr0+'=',
              maxstr = maxstr0+'=',
              ...
              IsHasFilter = (l_href.indexOf('filter.v.') !== -1),
              str_connect = (l_href.indexOf('?') !== -1) ? '&' : '?',
              IsHasFilterPrice = (l_href.indexOf('filter.v.price') !== -1);
              console.log(IsHasFilter,IsHasFilterPrice)

              if (IsHasFilterPrice) {

                currentUrlPrice = l_href.replace(minstr+getParameterByName(minstr0),minstr+currentMinPrice).replace(maxstr+getParameterByName(maxstr0),maxstr+currentMaxPrice)
              } else {
                 currentUrlPrice = l_href+str_connect+minstr+currentMinPrice+'&'+maxstr+currentMaxPrice
              }
              console.log(currentUrlPrice)
            //url1 = UpdateUrlFilter(l_href,'','filter.v.'),
   ...
   geckoShopify.RefreshPriceTitle = function ($el) {
      ...
      //_price_slider_wrapper.each(function() {
        ...
        var $stepsSlider = _this.find('.price_steps_slider:not(.noUi-target)'),
        ...
        stepsSlider.noUiSlider.on('update', function (values, handle) {
          //console.log(values, handle)
           ...
           _amount.find(inputs2[handle]).val(geckoShopify.formatMoney(value_curent,'{{amount}}').replaceAll(',',''));
...
jQuery_T4NT(document).ready(function($) {
  ...
  geckoShopify.flickityt4sResposition(true);
  //geckoShopify.recalculateSwatches();
  ...
  $('.lazy_menu_mega').one('lazyincluded', function(e) {
    ...
    if (el.find('.js_carousel.flickityt4s-enabled').length > 0 ) return;
    var owl = el.find('.js_carousel');
    geckoShopify.lazyWishUpdate();
    geckoShopify.refresh_flickity(owl);
    geckoShopify.flickityt4sResposition(false,owl);
    geckoShopify.recalculateSwatches();
  });
  ...
  .one('lazyincluded', function(e) {
    ...
    if (owl.length > 0) {
      ...
      owl.flickityt4s(JSON.parse(option));
      geckoShopify.flickityt4sResposition(false,owl);
      //geckoShopify.recalculateSwatches();
    }
